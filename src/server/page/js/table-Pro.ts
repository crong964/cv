...
function tableDataCell(data: Product) {
  return `
    <tr class="text-base">
    ...
    <td>
    <button class=" hover:bg-green-500 hover:text-white" onclick="loadChild(${
      data.idProduct
    })"> xem chi thêm </button>
    </td>
    </tr>`;
}
function multitableDataCell(data: Product[]) {
  var s = "";
...
function product(s: HTMLElement | undefined) {
  if (s) {
    removeActive();
    s.classList.add("active");
  }
  fetchGetUrl("/product/", (data: any) => {
    var tabledata = document.querySelector("#sampleTable") as HTMLInputElement;
    tabledata.innerHTML = table(multitableDataCell(data));
  });
}

function loadChild(idProduct: number) {
  fetchGetUrl(`/childproduct/${idProduct}`, (data: any[]) => {
    var tabledata = document.querySelector("#sampleTable") as HTMLInputElement;
