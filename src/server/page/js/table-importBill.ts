...
function ImportBillCell(data: ImportBill) {
  return `
      <tr class="text-base">
...
function ImportBillTable(data: any) {
  return `
...
function multitableImportBillCell(params: ImportBill[]) {
  var s = "";
...
var RemoveBill = (id: string) => {
  if (confirm("bạn muốn thực hiện")) {
    fetchPostUrl("/imPortBill/removeBill", { idBill: id }, (data: any) => {
